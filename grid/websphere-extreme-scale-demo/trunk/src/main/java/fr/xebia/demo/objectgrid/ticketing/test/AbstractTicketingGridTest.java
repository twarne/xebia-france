...
public abstract class AbstractTicketingGridTest extends TestCase {
    ...
    protected List<Long> persistedTrainIds = new ArrayList<Long>();
    ...
    protected void loadData(EntityManager entityManager) {
        ...
        for (int hour = 6; hour <= 24; hour++) {
            ...
            train.getTrainStops().add(new TrainStop(random.nextLong(), DateUtils.addHours(today, hour), PARIS_GARE_DE_LYON));
            train.getTrainStops().add(new TrainStop(random.nextLong(), DateUtils.addHours(today, 2 + hour), AVIGNON_TGV));
            train.getTrainStops().add(new TrainStop(random.nextLong(), DateUtils.addHours(today, 3 + hour), MARSEILLE_SAINT_CHARLES));
            for (int seatNumber = 1; seatNumber <= 10; seatNumber++) {
                train.getSeats().add(new Seat(random.nextLong(), seatNumber, random.nextBoolean(), 100 + random.nextInt(100)));
            }           
        ...
        for (int hour = 6; hour <= 24; hour++) {
            ...
            train.getTrainStops().add(new TrainStop(random.nextLong(), DateUtils.addHours(today, hour), MARSEILLE_SAINT_CHARLES));
            train.getTrainStops().add(new TrainStop(random.nextLong(), DateUtils.addHours(today, 1 + hour), AVIGNON_TGV));
            train.getTrainStops().add(new TrainStop(random.nextLong(), DateUtils.addHours(today, 3 + hour), PARIS_GARE_DE_LYON));
            for (int seatNumber = 1; seatNumber <= 10; seatNumber++) {
                train.getSeats().add(new Seat(random.nextLong(), seatNumber, random.nextBoolean(), 200 + random.nextInt(100)));
            }
    ...
    public ObjectGrid loadObjectGrid() throws ObjectGridException {
        ...
        if (useLocalGrid) {
            ...
            result.registerEntities(new Class[] {
                Train.class, TrainStop.class, Seat.class
            });
        } else {
    ...
    public void dumpTuple(Tuple tuple) {
        ...
        for (int i = 0; i < tupleMetadata.getNumAttributes(); i++) {
            ...
            System.out.println("\t\tname=" + tupleAttribute.getName() + ", type=" + tupleAttribute.getType() + ", alias="
                               + tupleAttribute.getAlias() + ", VALUE=" + tuple.getAttribute(i));
        }
        System.out.println("\tASSOCIATIONS");
        for (int i = 0; i < tupleMetadata.getNumAssociations(); i++) {
            TupleAssociation tupleAssociation = tupleMetadata.getAssociation(i);
            System.out.println("\t\tname=" + tupleAssociation.getName() + ", targetEntity=" + tupleAssociation.getTargetEntityName()
                               + ", type=" + tupleAssociation.getType() + ", targetEntityMetadata="
                               + tupleAssociation.getTargetEntityMetadata());
        }
    }
}
