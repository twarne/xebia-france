...
public class ObjectGridTemplate<Entity> {
    ...
    private final static Logger logger = Logger.getLogger(ObjectGridTemplate.class);
    ...
    public List<Entity> findByNamedParam(final String queryString, final String[] paramNames, final Object[] values) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("> distributedFind(query=" + queryString + ", paramNames=" + Arrays.asList(paramNames) + ", values='"
                    + Arrays.asList(values) + ")");
        }

        try {
            AgentManager agentManager = ObjectGridUtils.getCurrentSession(this.objectGrid).getMap(QueryAgent.AGENT_RUNNER_MAP_NAME)
                    .getAgentManager();
            ...
            throw new RuntimeException("Exception querying '" + queryString + "' with paramNames=" + Arrays.asList(paramNames)
                    + ", values=" + Arrays.asList(values) + " via the QueryAgent", e);
        } catch (UndefinedMapException e) {
            throw new RuntimeException("Map '" + QueryAgent.AGENT_RUNNER_MAP_NAME + "' used by QueryAgent to route queries was not found",
                    e);
        }
    }
