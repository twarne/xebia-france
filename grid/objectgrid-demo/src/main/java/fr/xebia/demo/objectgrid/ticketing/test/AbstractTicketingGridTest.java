...
public abstract class AbstractTicketingGridTest extends TestCase {
    ...
    private void loadData(EntityManager entityManager) {
        ...
        for (int hour = 6; hour <= 24; hour++) {
            ...
            train.getTrainStops().add(new TrainStop(random.nextInt(), DateUtils.add(today, Calendar.HOUR, hour), PARIS_GARE_DE_LYON));
            train.getTrainStops().add(new TrainStop(random.nextInt(), DateUtils.add(today, Calendar.HOUR, 2 + hour), AVIGNON_TGV));
            train.getTrainStops().add(
                    new TrainStop(random.nextInt(), DateUtils.add(today, Calendar.HOUR, 3 + hour), MARSEILLE_SAINT_CHARLES));
            for (int seatNumber = 1; seatNumber <= 10; seatNumber++) {
                train.getSeats().add(new Seat(random.nextInt(), seatNumber, random.nextBoolean()));
            }
        ...
        for (int hour = 6; hour <= 24; hour++) {
            ...
            train.getTrainStops().add(new TrainStop(random.nextInt(), DateUtils.add(today, Calendar.HOUR, hour), MARSEILLE_SAINT_CHARLES));
            train.getTrainStops().add(new TrainStop(random.nextInt(), DateUtils.add(today, Calendar.HOUR, 1 + hour), AVIGNON_TGV));
            train.getTrainStops().add(new TrainStop(random.nextInt(), DateUtils.add(today, Calendar.HOUR, 3 + hour), PARIS_GARE_DE_LYON));
            for (int seatNumber = 1; seatNumber <= 10; seatNumber++) {
                train.getSeats().add(new Seat(random.nextInt(), seatNumber, random.nextBoolean()));
            }
