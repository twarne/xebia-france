...
abstract class AbstractListenerContainerParser implements BeanDefinitionParser {
    ...
    protected static final String USE_MESSAGE_ID_IN_RESPONSE = "use-message-id-in-response";
    ...
    private void parseListener(Element listenerEle, Element containerEle, ParserContext parserContext) {
        ...
        if (listenerEle.hasAttribute(USE_MESSAGE_ID_IN_RESPONSE)) {
            Boolean  useMessageIdInResponse = Boolean.valueOf(listenerEle.getAttribute(USE_MESSAGE_ID_IN_RESPONSE));
            listenerDef.getPropertyValues().addPropertyValue("useMessageIdInResponse", useMessageIdInResponse);
        }           
